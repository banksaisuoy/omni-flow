...
export default function ShopPage() {
    ...
    const handleSearch = async (term: string) => {
        setLoading(true)
        setSearchError('')
        // Bound the server action so a cold database/provider cannot leave the shop stuck on "Thinking..." forever.
        const timeout = new Promise<never>((_, reject) => {
            setTimeout(() => reject(new Error('Product search timed out')), 12000)
        })

        try {
            const result = await Promise.race([smartSearchProducts(term), timeout])
            if (result.success) {
                ...
                setProducts([])
                setAiContext(null)
                setSearchError('Product catalog is temporarily unavailable. Please try again.')
            }
        } catch {
            setProducts([])
            setAiContext(null)
    ...
    return (
        <div className="min-h-screen pt-24 pb-12 px-4 md:px-8 max-w-7xl mx-auto">
            ...
            {searchError && (
                <div role="status" className="mb-8 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-200">
                    {searchError}
                </div>
            )}
